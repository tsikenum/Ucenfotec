...
let guardarFoto = () => {
    if (validarCampos() == false && validarEmail() == false && validarCedDigitos() == false) {
        console.log(widget_Cloudinary.open());
    }
};

let validarFoto = () => {
    let error = false;
    if (errorServicioImg == true) {
        error = true;
        Swal.fire({
            icon: "warning",
            title: "Fotografia faltante",
            text: "Por favor suba su fotografía",
        });
    }
    return error;
};
...
let enviarDatos = () => {
    ...
    let picture = fotoServicio;


    //primerNombre,segundoNombre,primerApellido,segundoApellido,tipoID,genero,idNumero,email,telefono,contrasena

    if (validarCampos() == false && validarEmail() == false && validarCedDigitos() == false && validarFoto() == false) {
        registrarEncargadoRuta(
            pNombre,
            sNombre,
            pApellido,
            sApellido,
...
btnFotografia.addEventListener("click", guardarFoto);
